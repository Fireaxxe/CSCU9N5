...
var W, H, ctx, input, r, run, kx = 30, ky = 40;
var field = [], forces = [];
...
function animate() {
  ...
  forces[0].f = -input.f.value;
  forces[0].r = input.r.value;
  forces[1].x = W / 2 - W / 4 * Math.cos(t);
  forces[1].y = H / 2 + W / 4 * Math.sin(t);
  forces[1].f = input.f.value;
  forces[1].r = input.r.value;
  applyForces();
  applyColors();
